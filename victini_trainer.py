...
def main():
    ...
    # train_data = Parallel(n_jobs=4)(delayed(make_victini_data)(filepath) for filepath in tqdm(train_files[:8]))  # MINI
    if Path(victini_path).exists():
        victini.load_state_dict(torch.load(victini_path))
    for _ in range(reps):
        # random.shuffle(train_files)
        # train_data = Parallel(n_jobs=4)(delayed(make_victini_data)(filepath) for filepath in tqdm(train_files[:30]))  # MEDIUM
        train(victini, train_data, lr=0.001, weight_decay=0, discount=0)
